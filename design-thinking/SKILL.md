...
description: >
  ...
  USAR SIEMPRE que el usuario diga frases como "tengo un problema", "necesito resolver",
  "no sé qué hacer con", "estoy atascado en", "quiero tomar una decisión sobre",
  "ayúdame a pensar en", "qué hago con", "analiza mi situación", "design thinking",
  "pensemos esto", o cuando describa un dilema profesional, estratégico o personal
  que requiera un análisis profundo antes de actuar. También disparar cuando el usuario
  mencione que debe tomar una decisión importante o que algo no está funcionando como esperaba.
---

# Design Thinking — Pensamiento Estructurado desde Cero

Este skill guía un proceso de análisis en 5 fases secuenciales. Cada fase se completa con el usuario antes de avanzar a la siguiente. **No saltes fases. No des consejos genéricos.**
...
```
PROBLEMA:  [enunciado]
↓ ¿Por qué ocurre eso? (1)  → [respuesta del usuario]
↓ ¿Por qué ocurre eso? (2)  → [respuesta del usuario]
↓ ¿Por qué ocurre eso? (3)  → [respuesta del usuario]
↓ ¿Por qué ocurre eso? (4)  → [respuesta del usuario]
↓ ¿Por qué ocurre eso? (5)  → [respuesta del usuario]
...
Primero preguntar: *"¿Cuál es la decisión que vas a tomar a partir de este análisis?"*

Luego trazar las consecuencias en **ambos escenarios**, en 3 capas temporales:
...
- **No des el análisis en un solo bloque.** Trabaja fase a fase, esperando la respuesta del usuario antes de avanzar.
...
- **Cerrar con síntesis.** Al final del proceso completo, ofrecer un resumen ejecutivo de 5-7 líneas: problema real, causa raíz, decisión recomendada, mayor riesgo y primera acción.
